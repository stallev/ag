...
- **Данные**:
  ...
  - OpenSearch — полнотекстовый поиск по материалам и описаниям ресурсов.
...
- Поиск материалов — запрос в AppSync → OpenSearch.
...
## Потоки и интеграции
- **Контент**: админка → AppSync mutations → DynamoDB; медиа → S3; после публикации — индекс в OpenSearch (тригер Lambda или AppSync pipeline resolver).
- **Обращения**: форма → AppSync → DynamoDB; notification Lambda → SendGrid/SES; модерация через админку.
- **Поиск**: запрос с фильтрами (category/tag) → AppSync → OpenSearch → результаты на фронтенде.
